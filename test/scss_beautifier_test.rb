...
class ScssBeautifierTest < Minitest::Test
  ...
  def test_formatters
    ...
    formatter_list.each do |f|
      f["files"].each do |file|
        assert test_style_rule(Object.const_get(f["formatter"]), file), "#{f["formatter"]} failed on #{file}"
      end
    end
  end
  ...
  def test_converter
    ...
    assert expected_contents == results.strip, "SCSSBeautifier::Convert failed"
  end
end
